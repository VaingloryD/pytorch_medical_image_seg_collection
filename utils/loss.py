...
class DiceLoss(nn.Module):
    ...
    def forward(self,input, target):
        ...
        intersect=(input*target).sum()#两者相交
        union = torch.sum(input) + torch.sum(target)#各自的元素相加
        Dice=(2*intersect+self.smooth)/(union+self.smooth)
        dice_loss=1-Dice
        return dice_loss

class Multi_DiceLoss(nn.Module):
    def __init__(self, class_num=3,smooth=0.1):
        super(Multi_DiceLoss, self).__init__()
        self.smooth = smooth
        self.class_num = class_num

    def forward(self,input, target):
        input = torch.exp(input)#返回一个新张量，包含输入input张量每个元素的指数。log(2)输出就是2
        
        Dice = Variable(torch.Tensor([0]).float()).cuda()
        for i in range(0,self.class_num):
            input_i = input[:,i,:,:]#将预测图的几类的标签分别拿出
            target_i = (target == i).float()#将金标准的三类标签拿出，对应的位置为1，其余的为0
